    ...
    <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:landscape:grid-cols-3 lg:grid-cols-3 gap-4 md:gap-6 items-start">
        ...
        <div className="space-y-6">
          ...
          {/* In-Game Chat */}
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 flex flex-col h-[300px] shadow-xl">
            <h3 className="text-sm font-extrabold text-white flex items-center gap-2 border-b border-slate-800 pb-2 mb-3">
              <MessageSquare className="w-4 h-4 text-amber-400" /> In-Game Chat
            </h3>

            <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
              {gameChatMessages.length === 0 ? (
                <div className="text-xs text-slate-500 text-center py-8">
                  No messages in match chat yet.
                </div>
              ) : (
                gameChatMessages.map((msg) => (
                  <div
                    key={msg.id}
                    className="bg-slate-950/80 p-2.5 rounded-xl border border-slate-800 text-xs"
                  >
            ...
            <form onSubmit={handleChatSubmit} className="mt-3 flex gap-2">
              ...
              <button
                ...
                className="px-3 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-xl transition"
              >
