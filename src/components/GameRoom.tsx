    ...
    <div className="w-full h-screen max-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between p-2 sm:p-3 relative select-none overflow-hidden animate-fade-in">
      ...
      {latestEmojiReaction && (
        <div className="absolute top-14 left-1/2 -translate-x-1/2 z-50 bg-slate-900/95 border-2 border-amber-400 px-4 py-1.5 rounded-full shadow-2xl flex items-center gap-2.5 animate-bounce">
          <span className="text-xl sm:text-2xl">{latestEmojiReaction.emoji}</span>
      ...
      <div className="flex items-center justify-between gap-2 bg-slate-900/90 border border-slate-800 px-3 py-1.5 rounded-2xl shadow-xl shrink-0">
        <button
          onClick={onLeaveRoom}
          className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs border border-slate-700 transition"
        >
      ...
      <div className="flex-1 my-1 flex flex-row items-center justify-between gap-2 sm:gap-3 overflow-hidden relative min-h-0">
        ...
        <div className="w-28 sm:w-40 shrink-0 h-full flex flex-col justify-between gap-1.5 bg-slate-900/90 border border-slate-800/90 p-2 rounded-2xl shadow-xl overflow-hidden min-h-0">
          {/* Black Player Summary */}
          <div className={`p-1.5 rounded-xl border text-xs ${room.currentTurn === 'black' ? 'bg-slate-950 border-amber-500/80' : 'bg-slate-950/60 border-slate-800'}`}>
            <div className="flex items-center gap-1.5 min-w-0">
              <span className="w-2.5 h-2.5 rounded-full bg-slate-900 border border-slate-400 shrink-0" />
              <span className="font-extrabold text-slate-100 text-[10px] sm:text-[11px] truncate">
                {room.blackPlayer?.username || 'Black'}
              </span>
            </div>
            <div className="text-[9px] sm:text-[10px] text-amber-400 font-bold mt-0.5">
          ...
          <div className="flex-1 my-0.5 flex flex-col overflow-hidden bg-slate-950/80 rounded-xl border border-slate-800/80 p-1.5 min-h-0">
            <div className="text-[9px] sm:text-[10px] font-extrabold text-amber-400 uppercase tracking-wider border-b border-slate-800 pb-0.5 flex items-center justify-between">
              <span>Moves</span>
              <span>#{room.history.length}</span>
            </div>
            <div className="flex-1 overflow-y-auto space-y-1 pt-1 custom-scrollbar min-h-0">
              {room.history.length === 0 ? (
                <div className="text-[9px] sm:text-[10px] text-slate-500 text-center py-2 italic">
                  Match start
                </div>
              ) : (
                room.history.map((m, idx) => (
                  <div
                    key={m.id}
                    className="flex items-center justify-between text-[9px] sm:text-[10px] p-0.5 sm:p-1 bg-slate-900 rounded border border-slate-800/80 font-mono"
                  >
                    ...
                    {m.capturedCount > 0 && <span className="text-amber-400 font-extrabold text-[8px]">x{m.capturedCount}</span>}
          ...
          {/* Red Player Summary */}
          <div className={`p-1.5 rounded-xl border text-xs ${room.currentTurn === 'red' ? 'bg-slate-950 border-amber-500/80' : 'bg-slate-950/60 border-slate-800'}`}>
            <div className="flex items-center gap-1.5 min-w-0">
            ...
            <div className="text-[9px] sm:text-[10px] text-rose-400 font-bold mt-0.5">
        ...
        <div className="shrink-0 flex items-center h-full min-h-0">
          ...
          >
            ...
            {isEmojiSidebarOpen ? <ChevronRight className="w-3.5 h-3.5" /> : <ChevronLeft className="w-3.5 h-3.5" />}
          </button>

          {/* Pop-Out Vertical Sidebar - 1 Column Format */}
          {isEmojiSidebarOpen && (
            <div className="h-full bg-slate-900 border border-amber-500/30 rounded-2xl p-1.5 shadow-2xl flex flex-col items-center justify-between gap-1 w-12 sm:w-14 animate-fade-in z-20 min-h-0">
              <div className="text-[8px] sm:text-[9px] font-extrabold text-amber-400 uppercase tracking-widest text-center border-b border-slate-800 pb-1 w-full">
                Emoji
              </div>

              {/* Vertical Scrollable Column of 10 Emojis */}
              <div className="flex-1 overflow-y-auto flex flex-col gap-1.5 my-0.5 custom-scrollbar w-full items-center min-h-0">
                {REACTION_EMOJIS.map((emoji, idx) => (
                  <button
                    key={idx}
                    onClick={() => onSendGameChat(emoji)}
                    className="w-8 h-8 sm:w-9 sm:h-9 text-lg sm:text-xl flex items-center justify-center rounded-xl bg-slate-950 hover:bg-amber-500/30 hover:scale-110 border border-slate-800 transition active:scale-90 shadow shrink-0"
                    title={`React ${emoji}`}
                  >
                    {emoji}
                  </button>
              ...
              <div className="text-[7px] sm:text-[8px] text-slate-500 font-bold text-center">
                React
              </div>
            </div>
          )}
